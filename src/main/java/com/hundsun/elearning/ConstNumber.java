 ...
 *               ②输出出现次数最多的数字，出现几次？如果次数相同，输出数字最大的一个。 注：利用HashMap实现次数的计算例如给定数组为
 *               [11, 164, 25, 34, 9, 32, 8, 11, 25, 7, 9, 25, 4, 25, 3, 19, 65,
 *               8, 9, 85, 9, 7, 25, 9, 12, 63]， 则①的结果为25，出现5次，②的结果为5，出现7次。
...
public class ConstNumber {
	...
	private static Map.Entry<Integer, Integer> getIntegerIntegerEntry(Integer[] arr) {
		...
		Map.Entry<Integer, Integer> integerIntegerEntry = hashMap.entrySet().stream()
				.filter(e -> e.getValue() == hashMap.values().stream().max(Integer::compareTo).get())
	...
	public static Map.Entry<Integer, Integer> getTheMap(Integer[] array) {
		...
		Map.Entry<Integer, Integer> integerIntegerEntry = hashMap.entrySet().stream()
				.filter(e -> e.getValue().equals(hashMap.values().stream().max(Integer::compareTo).get()))
