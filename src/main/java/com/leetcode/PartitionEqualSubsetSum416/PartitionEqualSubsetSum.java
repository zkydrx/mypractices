...
/**
 * Created with IntelliJ IDEA. User: Abbot Date: 2017-12-25 Time: 10:05
...
public class PartitionEqualSubsetSum {
	...
	/**
	 * 那个这个数组的所有和一定是个偶数才能满足条件。 这样问题就简单很多。
	 *
	 * @param nums
	 * @return
	 */
	public boolean canPartition(int[] nums) {
		...
		if (sum % 2 == 0) {
			...
			for (int i = 0; i < nums.length; i++) {
				/**
				 * 问题是不一定有一个数单独分为一组。 也就是说每组数的个数不能确定。
				 */
				if (nums[i] == temp) {
					count++;
				}
	...
	/**
	 * 背包问题（动态规划) 有bug,对于{3,3,3,4,5} 无法给出正确解答。
	 *
	 * @param nums
	 */
