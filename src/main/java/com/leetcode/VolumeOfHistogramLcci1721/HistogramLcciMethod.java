...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2021-03-16 Time: 14:05:18
 * Description: 题目地址：https://leetcode-cn.com/problems/volume-of-histogram-lcci/
 * 面试题 17.21. 直方图的水量 给定一个直方图(也称柱状图)，假设有人从上面源源不断地倒水，最后直方图能存多少水量?直方图的宽度为 1。
 * <p>
 * <p>
 * <p>
...
public class HistogramLcciMethod {
	...
	public void test1() {
		...
		for (int i = 1; i < arr.length - 1; i++) {
			...
			water += (Math.min(theLeftMaxNumber, theRightMaxNumber) - arr[i]) >= 0
					? (Math.min(theLeftMaxNumber, theRightMaxNumber) - arr[i])
					: 0;
		}
	}
	...
	/**
	 * 方法2.通过预处理数组 通过一次循环，生成左数组（递增数组，里面的每一位都是原数组下标坐标最大的数字。）
	...
	public void test2() {
		...
		for (int i = 0; i < arr.length; i++) {
			water += (Math.min(leftArr[i], rightArr[i]) - arr[i]) > 0
					? (Math.min(leftArr[i], rightArr[i]) - arr[i])
					: 0;
		}
	}

	/**
	 * 方法3：双指针最优解。 左指针从1开始中间移动 右指针从倒数第二位向中间移动 左右指针相遇结束整个遍历。
	 */
	@Test
	public void test3() {
		int[] arr = {0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1};
