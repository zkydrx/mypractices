...
/**
 * Created with IntelliJ IDEA. User: Abbot Date: 2017-09-12 Time: 15:56
 * Description: 写一个程序来检测一个整数是不是丑数。
 * <p>
 * 丑数的定义是，只包含质因子 2, 3, 5 的正整数。比如 6, 8 就是丑数，但是 14 不是丑数以为他包含了质因子 7。 样例 给出 num =
 * 8，返回 true。 给出 num = 14，返回 false。
 */
public class UglyNumber {
	...
	public boolean isUgly(int number) {
		...
		boolean b = false;
		/**
		 * 用三层循环来求幂指数，将三个底的指数结果进行乘积运算， 并判断是否满足与给定的值相等，如果相等则记录布尔值为真，并且 退出三层循环。
		 * 返回结果。否者直至循环结束不对初始的布尔值进行值得修改。 最终返回布尔值。就是我们判断的结果。
		 */
		lab : for (int i = 0; i <= number; i++) {
			num2 = Math.pow(2, i);
