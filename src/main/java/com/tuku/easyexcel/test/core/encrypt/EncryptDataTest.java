package com.tuku.easyexcel.test.core.encrypt;

import com.alibaba.excel.EasyExcel;
import com.alibaba.excel.support.ExcelTypeEnum;
import com.tuku.easyexcel.test.core.simple.SimpleData;
...
public class EncryptDataTest
{
    ...
    private void readAndWriteStream(File file, ExcelTypeEnum excelType) throws Exception
    {
        ...
        EasyExcel.write(fileOutputStream, EncryptData.class).password("123456").excelType(excelType).sheet().doWrite(data());
        fileOutputStream.close();

        FileInputStream fileInputStream = new FileInputStream(file);
        EasyExcel.read(fileInputStream, EncryptData.class, new EncryptDataListener()).password("123456").sheet().doRead();
    }
