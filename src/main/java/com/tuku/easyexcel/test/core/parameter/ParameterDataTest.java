package com.tuku.easyexcel.test.core.parameter;
...
public class ParameterDataTest
{
    ...
    private void readAndWrite1()
    {
        ...
        EasyExcel.read(file.getPath()).head(ParameterData.class).registerReadListener(new ParameterDataListener()).sheet().doRead();
    }
    ...
    {
        ExcelWriter excelWriter = EasyExcel.write(new FileOutputStream(file)).head(ParameterData.class).relativeHeadRowIndex(0).build();
