 ...
 * ====================================================================
 */
package com.tuku.easyexcel.test.temp.dataformat;
 ...
 * DataFormatter.setDefaultNumberFormat(Format)</code>. <b>Note:</b> the default
 * format will only be used when a Format cannot be created from the cell's data
 * format string.
 *
 * <p>
 * Note that by default formatted numeric values are trimmed. Excel formats can
 * contain spacers and padding and the default behavior is to strip them off.
 * </p>
 * <p>
 * Example:
 * </p>
 ...
 * <code>"##.##_ "</code>. The trailing underscore and space ("_ ") in the
 * format adds a space to the end and Excel formats this cell as
 * <code>"12.34 "</code>, but <code>DataFormatter</code> trims the formatted
 * value and returns <code>"12.34"</code>.
 * </p>
 * You can enable spaces by passing the <code>emulateCSV=true</code> flag in the
 * <code>DateFormatter</code> cosntructor. If set to true, then the output tries
 * to conform to what you get when you take an xls or xlsx in Excel and Save As
 * CSV file:
 * <ul>
 * <li>returned values are not trimmed</li>
 * <li>Invalid dates are formatted as 255 pound signs ("#")</li>
 * <li>simulate Excel's handling of a format string of all # when the value is
 * 0. Excel will output "", <code>DataFormatter</code> will output "0".
 * </ul>
 * <p>
 * Some formats are automatically "localized" by Excel, eg show as mm/dd/yyyy
 * when loaded in Excel in some Locales but as dd/mm/yyyy in others. These are
 * always returned in the "default" (US) format, as stored in the file. Some
 * format strings request an alternate locale, eg
 * <code>[$-809]d/m/yy h:mm AM/PM</code> which explicitly requests UK locale.
 * These locale directives are (currently) ignored. You can use
 * {@link DateFormatConverter} to do some of this localisation if you need it.
 */
public class DataFormatter1 implements Observer {
	 ...
	 */
	private static final Pattern rangeConditionalPattern = Pattern
			.compile(".*\\[\\s*(>|>=|<|<=|=)\\s*[0-9]*\\.*[0-9].*");

	/**
	 * A regex to find locale patterns like [$$-1009] and [$?-452]. Note that we
	 * don't currently process these into locales
	 */
	...
	private static final Pattern colorPattern = Pattern.compile(
			"(\\[BLACK\\])|(\\[BLUE\\])|(\\[CYAN\\])|(\\[GREEN\\])|" + "(\\[MAGENTA\\])|(\\[RED\\])|(\\[WHITE\\])|"
					+ "(\\[YELLOW\\])|" + "(\\[COLOR\\s*\\d\\])|(\\[COLOR\\s*[0-5]\\d\\])",
			Pattern.CASE_INSENSITIVE);

	/**
	 * A regex to identify a fraction pattern. This requires that replaceAll("\\?",
	...
	/**
	 * A regex to detect if an alternate grouping character is used in a numeric
	 * format
	 */
	private static final Pattern alternateGrouping = Pattern.compile("([#0]([^.#0])[#0]{3})");

	/**
	 * Cells formatted with a date or time format and which contain invalid date or
	 * time values show 255 pound signs ("#").
	 */
	...
	/**
	 * stores if the locale should change according to
	 * {@link LocaleUtil#getUserLocale()}
	 */
	 ...
	 * one. This method will return <code>null</code> if the any of the following is
	 * true:
	 * <ul>
	...
	private Format getFormat(double cellValue, int formatIndex, String formatStrIn) {
		...
		// Might be better to separate out the n p and z formats, falling back to p when
		// n and z are not set.
		...
		// Excel's # with value 0 will output empty where Java will output 0. This hack
		// removes the # from the format.
	...
	/**
	 * Create and return a Format based on the format string from a cell's style. If
	 * the pattern cannot be parsed, return a default pattern.
	 *
	 * @param cell
	 *            The Excel cell
	...
	private Format createFormat(double cellValue, int formatIndex, String sFormat) {
		...
		// Strip off the locale information, we use an instance-wide locale for
		// everything
		...
		if (formatStr.contains("#/") || formatStr.contains("?/")) {
			...
			// Strip custom text in quotes and escaped characters for now as it can cause
			// performance problems in
			// fractions.
			// String strippedFormatStr = formatStr.replaceAll("\\\\ ", "
			// ").replaceAll("\\\\.",
	...
	private Format createDateFormat(String pFormatStr, double cellValue) {
		...
		// From Excel help:
		/*
		 * The "m" or "mm" code must appear immediately after the "h" or"hh" code or
		 * immediately before the "ss" code; otherwise, Microsoft Excel displays the
	...
	/**
	 * Returns the formatted value of an Excel date as a <tt>String</tt> based on
	 * the cell's <code>DataFormat</code>. i.e. "Thursday, January 02, 2003" ,
	 * "01/02/2003" , "02-Jan" , etc.
	 * <p>
	 * If any conditional format rules apply, the highest priority with a number
	 * format is used. If no rules contain a number format, or no rules apply, the
	 * cell's style format is used. If the style does not have a format, the default
	 * date format is applied.
	 *
	 * @param cell
	 *            to format
	 * @param cfEvaluator
	...
	/**
	 * Returns the formatted value of an Excel number as a <tt>String</tt> based on
	 * the cell's <code>DataFormat</code>. Supported formats include currency,
	 * percents, decimals, phone number, SSN, etc.: "61.54%", "$100.00", "(800)
	 * 555-1234".
	 * <p>
	...
	/**
	 * <p>
	 * Returns the formatted value of a cell as a <tt>String</tt> regardless of the
	 * cell type. If the Excel format pattern cannot be parsed then the cell value
	 * will be formatted using a default format.
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * Returns the formatted value of a cell as a <tt>String</tt> regardless of the
	 * cell type. If the Excel number format pattern cannot be parsed then the cell
	 * value will be formatted using a default format.
	 * </p>
	 * <p>
	 * When passed a null or blank cell, this method will return an empty String
	 * (""). Formula cells will be evaluated using the given
	 * {@link FormulaEvaluator} if the evaluator is non-null. If the evaluator is
	 * null, then the formula String will be returned. The caller is responsible for
	 * setting the currentRow on the evaluator
	 * </p>
	 *
	 * @param cell
	 ...
	 * is used. If no ConditionalFormattingEvaluator is present, no rules apply, or
	 * the applied rules do not define a format, the cell's style format is used.
	 * </p>
	 * <p>
	 ...
	 * <code>java.text.Format#format</code>) will be a double value from a numeric
	 * cell. Therefore the code in the format method should expect a
	...
	/**
	 * Enables excel style rounding mode (round half up) on the Decimal Format
	 * given.
	 */
	...
	/**
	 * If the Locale has been changed via {@link LocaleUtil#setUserLocale(Locale)}
	 * the stored formats need to be refreshed. All formats which aren't originated
	 * from DataFormatter itself, i.e. all Formats added via
	 * {@link DataFormatter#addFormat(String, Format)} and
	 * {@link DataFormatter#setDefaultNumberFormat(Format)}, need to be added again.
	 * To notify callers, the returned {@link Observable} should be used. The Object
	 * in {@link Observer#update(Observable, Object)} is the new Locale.
	 *
	...
	/**
	 * Format class for Excel's SSN format. This class mimics Excel's built-in SSN
	 * formatting.
	 *
	 * @author James May
	 */
	 ...
	 * <p>
	 * This format is used to simulate Excel's handling of a format string of all #
	 * when the value is 0. Excel will output "", Java will output "0".
	 */
	@SuppressWarnings("serial")
