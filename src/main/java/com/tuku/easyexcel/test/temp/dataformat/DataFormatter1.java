...
public class DataFormatter1 implements Observer
{
     ...
     */
    private static final Pattern colorPattern = Pattern.compile("(\\[BLACK\\])|(\\[BLUE\\])|(\\[CYAN\\])|(\\[GREEN\\])|" + "(\\[MAGENTA\\])|(\\[RED\\])|(\\[WHITE\\])|" +
    ...
    private Format createFormat(double cellValue, int formatIndex, String sFormat)
    {
        ...
        while (m.find())
        {
            ...
            if (symbol.indexOf('$') > -1)
            {
                symbol = symbol.substring(0, symbol.indexOf('$')) + '\\' + symbol.substring(symbol.indexOf('$'), symbol.length());
            }
