...
public class DouBanTop250
{
    public static void main(String[] args) throws IOException
    {
        ...
        EasyExcel.write(fileName, DouBanData.class).sheet("豆瓣TOP250电影").registerWriteHandler(new WriteHandler()).doWrite(list1);
    ...
    private static void getTheResultString(String string, List<DouBanWriteData> list1) throws IOException
    {
        ...
        for (Element element : infos)
        {
            ...
            DouBanWriteData douBanWriteData = DouBanWriteData.builder().link(link).title(title).score(score).num(num).quote(quoteStr).build();
