...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2019-10-27 Time: 10:36:02
 * Description:
 */
public class ThreadPoolExecutorTests {
	...
	public void testThreadPoolExecutor2() {
		ThreadPoolExecutor pool = new ThreadPoolExecutor(2, 2, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(10),
	 ...
	 * submit的坑
	 * <p>
	 * 运行结果如下： 100.0 25.0 33.0 50.0
	 * <p>
	 * 可以看出运行结果为4个，因该是有5个的，但是当i=0的时候，100/0是会报错的，但是日志信息中没有任何信息，
	 * 是为什么那？如果使用了submit(Runnable task) 就会出现这种情况，任何的错误信息都出现不了！ 这是因为使用submit(Runnable
	 * task) 的时候，错误的堆栈信息跑出来的时候会被内部捕获到， 所以打印不出来具体的信息让我们查看，解决的方法有如下两种：
	 * 1、使用execute（）代替submit（）；参考testThreadPoolExecutor5() 2、使用Future
	 * 参考testThreadPoolExecutor6()
	 */
	@Test
	 ...
	 * <p>
	 * <p>
	 * 使用execute()方法 将会捕获异常信息，并打印出异常的堆栈信息
	 */
	@Test
