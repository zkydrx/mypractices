...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2019-11-13 Time: 18:57:22
 * Description:
 */
public class ComputeTest {
	/**
	 * Java实现约瑟夫环：50个人围成一圈数数，从1开始报数，数到3的人退出，求最后剩下的人的编号 利用可变数组ArrayList实现。
	 * 算法思想：将所有人的报数放到链表内；利用链表，将链表表头挨个移除， 并放到表的后面，在放第三个元素的时候，直接删除掉
	 * （如果有要求，可以删除的同时打印该元素，该元素即为出队人的编号）。
	 * <p>
	 * 约瑟夫环：已知n个人(以编号1，2，3...n分别表示)围坐在一张圆桌周围。 从编号为k的人开始报数，数到m的那个人出列;他的下一个人又从1开始报数，
	 * 数到m的那个人又出列;依此规律重复下去，直到圆桌周围的人全部出列。 通过list把留下的元素放到列表的最后边，然后删除首个元素也就是满足条件的元素，
	 * 直到列表只剩下一个元素停止循环。 这个是最简单的办法。目前我所知道的。
	 *
	 * @param n
	 *            人的总数
	 * @param m
