...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2019-11-17 Time: 17:53:58
 * Description:
 */
class VolatileData {
	int number = 0;
...
public class VolatileTest {
	/**
	 * 1.验证Volatile的可见性 1.1假如int number=0,number 没有volatile关键字修饰
	...
	public void testVolatile() {
		...
		new Thread(() -> {
			...
			volatileData.add();
			System.out.println(
					Thread.currentThread().getName() + "\t update the number finshed. number=" + volatileData.number);
			System.out.println(
					Thread.currentThread().getName() + "\t update the number finshed. number1=" + volatileData.number1);
		}, "A").start();
	...
	/**
	 * 2.验证volatile不保证原子性问题 2.1原子性概念：不可分割，完整性，某个线程正在做某个业务时，中间不能加塞或者分割， 要么同时成功要么同时失败。
	 * 3.如何解决原子性问题 3.1 使用synchronized 同步锁 3.2 使用juc的atomicInteger原子类
	 */
	@Test
	public void testAtomic() {
		...
		System.out.println(
				Thread.currentThread().getName() + "\t finally the atomicInteger=" + volatileData.atomicInteger);
	}
