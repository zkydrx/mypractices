 ...
 * <p>
 * }
 * 不管成功与否，都会调用 System.exit() 方法关闭程序，
 * 这个方法是用来结束当前正在运行中的java虚拟机。
 * System.exit(0) 是正常退出程序，
 * 而 System.exit(1) 或者说非0表示非正常退出程序。
 * 由此可见，junit 并不适合用来测试多线程程序呢，
 * 但是也不是没有方法，根据其原理可以尝试让主线程阻塞一下，
 * 等待其他子线程执行完毕再继续。
 * 最简单的方法就是让主线程睡眠个几秒钟：
 ...
 * 再在每个线程里调用这个方法： countDownLatch.countDown();
 * try
 * {
...
public class ContainerNotSafeTest
{
     ...
     * 因为当前容器不会添加任何元素。所以CopyOnWrite 也是一种读写分离的思想，读和写不同的容器。
     * 底层源码：
