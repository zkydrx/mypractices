...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2019-11-21 Time: 20:16:18
 * Description: synchronized ,ReentrantLock 是非公平锁，可重入锁，独占锁
...
public class LockTest {
	 ...
	 * 同一个线程外层函数获取锁之后，内层函数仍然能够获取该锁的代码 在同一个线程外层方法获取锁的时候，再进入内层方法会自动获取锁
	 * 也就是说线程可以进入任何一个他已经拥有锁所同步着的代码块
	 * <p>
	 * t1 invoke work() t1 invoke eat() t2 invoke work() t2 invoke eat()
	 * 证明synchronized是可重入锁。
	 */
	@Test
	...
	/**
	 * t3 invoke look() t3 invoke listen() t4 invoke look() t4 invoke listen()
	...
	/**
	 * 手写一个自旋锁 自旋锁的好处：循环比较直到成功为止，没有类似于wait的阻塞
	 * 通过cas完成自旋锁，A线程先进来调用myLock自己持有锁5秒钟，B线程进来发现当前线程持有锁，所以只能自旋等待 直到A释放锁B随后抢到
	 */
	@Test
	...
	/**
	 * 读写锁：ReentrantReadWriteLock 多个线程同时读取一个资源是没有任何问题的，为了满足高并发，读取线程可同时进行
	 * 如果有一个线程想去写资源的话，就不应该有其他线程对该资源进行读或者写 总结如下： 读-读可共存 读-写不可共存 写-写不可共存
