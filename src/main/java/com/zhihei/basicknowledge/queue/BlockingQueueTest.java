...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2019-11-23 Time: 23:09:21
 * Description: ArrayBlockingQueue:是一个基于数组结构的有界阻塞队列，此队列按FIFO(先进先出)原则对元素进行排序
 * LinkedBlockingQUeue:一个基于链表结构的阻塞队列，此队列按FIFO(先进先出)排序元素，吞吐量通常要高于ArrayBlockingQueue
 * SynchronousQueue:一个不存储元素的阻塞队列。每次插入数据必须要等到另外一个线程调用移除操作，否则插入操作一直处于阻塞状态 1.队列
 * 2.阻塞队列 2.1阻塞队列有没有好的一面 2.2.如果不得不阻塞，应该如何处理？
 */
public class BlockingQueueTest {
	...
	public void testAddRemoveElement() {
		...
		/**
		 * 返回队列头的元素，若头元素不存在则抛异常 java.util.NoSuchElementException
		 */
		System.out.println(blockingQueue.element());

		/**
		 * 当队列满时再往队列里加新元素则会跑出异常 java.lang.IllegalStateException: Queue full
		 */
		// System.out.println(blockingQueue.add("3"));
		System.out.println(blockingQueue.remove());
		System.out.println(blockingQueue.remove());
		/**
		 * 当删除的元素超过队列长度时，将报异常： java.util.NoSuchElementException
		 */
		// System.out.println(blockingQueue.remove());
	...
	/**
	 * 越界给出特殊值， offer->false poll->null peek->null
	 */
	@Test
	...
	/**
	 * SynchronousQueue没有容量 每一个put操作必须要等待以个take操作，否则不能继续添加元素，但是不阻塞
	 *
