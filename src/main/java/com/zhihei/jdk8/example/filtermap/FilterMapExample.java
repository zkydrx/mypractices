...
public class FilterMapExample
{
    ...
    public static void main(String[] args)
    {
        ...
        map.entrySet().stream().filter(item->"google.com".equals(item.getValue())).forEach(item -> System.out.println(item.getValue()));
        System.out.println("将获取到的value串联起来:");
        String collect = map.entrySet().stream().filter(item -> "google.com".equals(item.getValue())).map(item -> item.getValue()).collect(Collectors.joining());
        System.out.println(collect);

        System.out.println("获取指定key的元素");
        map.entrySet().stream().filter(item-> 2==item.getKey()).collect(Collectors.toMap(k-> k.getKey(),k->k.getValue())).forEach((k,v)-> System.out.println(k+"::"+v));
    }
}
