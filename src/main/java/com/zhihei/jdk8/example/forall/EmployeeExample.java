...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2019-11-10 Time: 12:44:18
 * Description:
 */
public class EmployeeExample {
	...
	public static void main(String[] args) {
		 ...
		 */
		List<Employee> collect8 = employees.stream().filter(employee -> "HangZhou".equals(employee.getOffice())
				&& employee.getAge() > 16 && employee.getSalary() > 21000).collect(Collectors.toList());
		collect8.forEach(System.out::println);

		/**
		 * 返回所有雇员的姓名和年龄
		 */
		Map<String, Integer> collect9 = employees.stream()
		 ...
		 */
		List<Employee> hangZhou2 = employees.stream().filter(employee -> employee.getOffice().equals("HangZhou"))
				.collect(Collectors.toList());

		System.out.println("查找办公司的地点是HangZhou的雇员方法:");
		hangZhou2.forEach(System.out::println);
		Optional<Employee> any = employees.stream().filter(employee -> "HangZhou".equals(employee.getOffice()))
				.findAny();
		System.out.println("查找办公司的地点是HangZhou的任意一个员工" + any);
		/**
		 * 按照工资的降序来列出员工信息
		 */
		List<Employee> collect3 = employees.stream().sorted((e1, e2) -> Integer.compare(e2.getSalary(), e1.getSalary()))
				.collect(Collectors.toList());

		System.out.println("按照工资的降序来列出员工信息方法1:");
		collect3.forEach(System.out::println);
		System.out.println("按照工资的降序来列出员工信息方法2:");
		List<Employee> collect5 = employees.stream().sorted((e1, e2) -> e2.getSalary() - e1.getSalary())
				.collect(Collectors.toList());
		...
		List<Employee> collect4 = employees.stream().sorted(Comparator.comparing(Employee::getName))
				.collect(Collectors.toList());
		 ...
		 * 获取工资最高的前两条雇员信息:
		 */
		List<Employee> collcet5 = employees.stream().sorted((e1, e2) -> Integer.compare(e2.getSalary(), e1.getSalary()))
				.collect(Collectors.toList()).subList(0, 2);

		System.out.println("获取工资最高的前两条雇员信息方法1:");
		collcet5.forEach(System.out::println);
		List<Employee> collect6 = employees.stream().sorted((e1, e2) -> Integer.compare(e2.getSalary(), e1.getSalary()))
		 ...
		 * 获取平均工资
		 */
		double average = employees.stream().collect(Collectors.summarizingInt(employees -> employees.getSalary()))
				.getAverage();
		 ...
		 * 查找HangZhou雇员的平均薪资
		 */
		OptionalDouble average3 = employees.stream().filter(employee -> "HangZhou".equals(employee.getOffice()))
		...
		Optional<Employee> hangZhou3 = employees.stream().filter(employee -> employee.getOffice().equals("HangZhou"))
				.findFirst();
		...
		employees.stream().map(employee -> employee.getName()).map(e -> e.substring(0, 1)).collect(Collectors.toList())
