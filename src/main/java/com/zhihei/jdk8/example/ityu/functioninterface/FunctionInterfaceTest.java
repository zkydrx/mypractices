...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2019-11-13 Time: 00:01:10
...
public class FunctionInterfaceTest {
	 ...
	 * 当需要通过一个方法来获取一个java.util.Comparator接口类型的对象作为排序器时,就可以调该方法获取。 使用函数式接口作为方法的返回值
	 *
	 * @return
	 */
	...
	/**
	 * Supplier<T> 供给型 Consumer<T> 消费型 Predicate<T> 断定型 Function<T,R> 函数型
	 */
	/**
	 * ①提供类型：Supplier接口 特点：只出不进，作为方法/构造参数、方法返回值
	...
	/**
	 * ②消费类型：Consumer接口 特点：只进不出，作为方法/构造参数
	...
	/**
	 * andThen: Consumer接口的默认方法andThen
	...
	/**
	 * 按照格式“姓名：XX。性别：XX。”的格式将信息打印 要求将打印姓名的动作作为第一个Consumer接口的Lambda实例，
	...
	/**
	 * ③断定类型：Predicate接口 特点：boolean类型判断，作为方法/构造参数 java.util.function.Predicate接口
	 * 作用:对某种数据类型的数据进行判断,结果返回一个boolean值
	 * <p>
	 * Predicate接口中包含一个抽象方法： boolean test(T t):用来对指定数据类型数据进行判断的方法 结果: 符合条件,返回true
	 * 不符合条件,返回false
	 */
	 ...
	 * <p>
	 * 1default Predicate<T> and(Predicate<? super T> other) { 2
	 * Objects.requireNonNull(other); 3 return (t) -> this.test(t) && other.test(t);
	 * 4}
	 *
	 * @param str
	 * @param predicate1
	 ...
	 * <p>
	 * 1 default Predicate<T> or(Predicate<? super T> other) { 2
	 * Objects.requireNonNull(other); 3 return (t) -> test(t) || other.test(t); 4 }
	 *
	 * @param str
	 * @param predicate1
	 * @param predicate2
	 ...
	 * andThen方法：
	 */
	public static String convertStringAndThen(String str, Function<String, Integer> function1,
	...
	/**
	 * String str = "赵丽颖,20"; 1.将字符串截取数字年龄部分，得到字符串； 2.将上一步的字符串转换成为int类型的数字；
	 * 3.将上一步的int数字累加100，得到结果int数字。
	 */
	public static int convertGroup(String str, Function<String, String> function1, Function<String, Integer> function2,
	...
	/**
	 * 方法引用 若lambda体中的内容，有方法已经实现了，则可以使用方法引用。方法引用是对lambda的简化
	 */
