...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2019-11-16 Time: 11:03:32
 * Description:
 */
public class MethodReferenceTest {
	...
	/**
	 * 方法引用 双冒号::为引用运算符，而它所在的表达式被称为方法引用。 如果Lambda要表达的函数方案已经存在于某个方法的实现中，那么则可以通过双冒号
	 * 来引用该方法作为Lambda的替代者。
	 *
	 * 三种格式：
	 *
	 ...
	 *
	 * 以上两种写法完全等效： 第一种：拿到参数之后通过Lambda传递给 System.out.println 方法去处理。 第二种：直接让
	 * System.out 中的 println 方法来取代Lambda。
	 *
	 * 注意:lambda体中调用的方法的参数列表和返回值类型要与函数式接口的抽象方法的参数列表与返回值类型一致。 Lambda 中
	 * 传递的参数一定是方法引用中的那个方法可以接收的类型,否则会抛出异常
	 */
	...
	public void testPrintStringByMethod() {
		...
		/**
		 * 使用方法引用： 1.MethodReadObj对象已经存在 2.成员方法printUppercaseString已经存在 所以可以使用对象名引用成员方法
		 */
		MethodReferenceObj methodReferenceObj = new MethodReferenceObj();
		printStringByMethod(methodReferenceObj::printUppercaseString);

		/**
		 * 3.2 通过类名引用静态方法 类已经存在，静态方法已经存在，则可以通过类名直接引用静态成员方法
		 */
	...
	/**
	 * 通过类名引用实例方法 注意：以上这种情况，需要满足一定的条件：lambda表达式中第一个参数是lambda体中的调用者，
	 * 第二个参数是lambda体中的参数
	 */
	@Test
	...
	/**
	 * 3.3 通过super引用成员方法 如果存在继承关系，当Lambda中需要出现super调用时，也可以使用方法引用进行替代。
	 */
	@Test
	...
	/**
	 * 3.4 通过this引用成员方法 this代表当前对象，如果需要引用的方法就是当前类中的成员方法， 那么可以使用 this::成员方法的格式来使用方法引用
	 */
	public void buyCar() {
	...
	public void doIt() {
		...
		/**
		 * 1. this已经存在 2. 本类的成员方法buyCar()已经存在 所以可以直接使用this引用本类的成员方法buyCar.
		 */
		getSalary(this::buyCar);
	}
	...
	/**
	 * 3.5类的构造器引用 由于构造器的名称与类名完全一样，并不固定。 所以构造器引用使用 类名称::new 的格式表示。
	 */
	...
	public void testPrintName() {
		...
		/**
		 * 使用方法引用： 1.构建方法 new Person(String name) 已知 2.创建对象已知 可以使用Person1的引用new创建对象
		 */
		printName("zky", 11, Person1::new);
	}

	/**
	 * .6 数组的构造器引用 数组也是 Object 的子类对象，所以同样具有构造器，只是语法稍有不同。 格式：Type[]::new
	 */
	@Test
	...
	public void testCreateArray() {
		...
		/**
		 * 1.已知创建的是int[]数组 2.创建数组长度也是已知的 使用方法引用，int[]数组引用new,根据参数传递的长度创建数组
		 */
