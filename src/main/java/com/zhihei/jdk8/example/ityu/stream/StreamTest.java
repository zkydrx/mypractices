 ...
 * 1.元素是特定类型的对象，形成一个队列。
 * 2.数据源流的来源。可以是集合，数组等。
 * 3.Stream自己不会存储元素，而是按需计算。
 * 4.Stream不会改变源对象，并且能返回一个持有结果的新流
 * 5.Stream操作是延迟操作，意味着他们会等到需要结果的时候才执行
 * 和以前的Collection操作不同， Stream操作还有两个基础的特征：
 * a.Pipelining: 中间操作都会返回流对象本身。这样多个操作可以串联成一个管道，
 * 如同流式风格（fluentstyle）。这样做可以对操作进行优化，
 * 比如延迟执行(laziness)和短路( short-circuiting)。
 * b.内部迭代：以前对集合遍历都是通过Iterator或者增强for的方式,
 ...
 * <p>
 * 注意：“Stream流”其实是一个集合元素的函数模型，它并不是集合，也不是数据结构，
 * 其本身并不存储任何元素（或其地址值）。
 */
public class StreamTest
{
    ...
    public void testGetStream()
    {
        ...
        /**
         * ②根据Map获取流
         * java.util.Map 接口不是Collection的子接口，且其K-V数据结构不符合流元素的单一特征，
     ...
     * （除了终结方法外，其余方法均为延迟方法。）
     * 终结方法：返回值类型不再是 Stream接口自身类型的方法，
     * 因此不再支持类似 StringBuilder 那样的链式调用。终结方法包括 count 和 forEach 方法。
     */
    ...
    public void testMethods()
    {
        ...
        /**
         * ③映射：map
         * 接收lambda，将元素转换成其他形式或提取信息，接收一个函数作为参数，该函数会被应用到每个元素上，
         * 并将其映射成一个新的元素。即将流中的元素映射到另一个流中。
         * 方法签名：
         *
         ...
         * 即对流扁平化处理,浅显一点解释就是把几个小的list转换到一个大的list
         * 如：[['a','b'],['c','d']] - > ['a','b','c','d']
         * 如果我们使用常用的map()方法获取的lowercaseWords数据结构为：
         * [['a','b','c'],['m','d','w'],['k','e','t']]。
         * 如果我们要得到如：['a','b','c','m','d','w','k','e','t']这样数据结构的数据，
         * 就需要使用flatMap()方法。
         */
        System.out.println("\n4测试flatMap");
         ...
         *
         * 1static <T> Stream<T> concat(Stream<? extends T> a, Stream<? extends T> b)
         * 2
     ...
     * 或者是一个Object[]。 例如我们有一个方法叫做test(String...strings)，
     * 那么你还可以写方法test()，但你不能写test(String[]
