package com.zhihei.multi.procuderconsumer;
...
public class Computer
{
    ...
    public synchronized void put(String name)
    {
        ...
        if (number >= 10)
        {
            ...
            System.out.println(Thread.currentThread().getName() + "：存放等待---仓库库存足够,此时库存是" + number);
            return;
        }
        number += 1;
    ...
    public synchronized void get(String name)
    {
        ...
        if (number <= 0)
        {
            ...
            System.out.println(Thread.currentThread().getName() + ": 取电脑受阻-因为库存余数为" + number);
            return;
        }
        number -= 1;
