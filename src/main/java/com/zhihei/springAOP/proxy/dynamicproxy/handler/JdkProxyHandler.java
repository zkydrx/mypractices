...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2019-11-07 Time: 21:13:25
 * Description:
 */
public class JdkProxyHandler {
	...
	public Object getProxyInstance() {
		/**
		 * Proxy.newProxyInstance() 方法，该方法接收三个参数： 第一个参数指定当前目标对象使用的类加载器,获取加载器的方法是固定的；
		 * 第二个参数指定目标对象实现的接口的类型； 第三个参数指定动态处理器， 执行目标对象的方法时,会触发事件处理器的方法。网上针对第三个参数的写法都是 new
		 * 一个匿名类来处理，我这直接用的 Java8 里面的 lamda 表达式来写的，都一样。底层原理使用的是反射机制
		 * 
		 */
		return Proxy.newProxyInstance(realBoss.getClass().getClassLoader(), realBoss.getClass().getInterfaces(),
				(proxy, method, args) -> {
