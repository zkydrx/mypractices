...
/**
 * Created with IntelliJ IDEA. Author: zky Date: 2021-02-24 Time: 16:08:40
 * Description: 验证synchronized的作用域。 修饰代码块：大括号括起来的代码，作用于调用的对象。
 * 修饰方法：整个方法，作用于调用的对象。 修饰静态方法：整个静态方法，作用于所有对象。 修饰类：括号括起来的部分，作用于所有对象。
 */
public class SyncTest {
	...
	/**
	 * 说明： 在本测试中开了两个线程调用同一个对象中的synchronized代码块，输出为先打印第一个线程的输出，再打印第二个线程的输出。
	 */
	@Test
	public void testOne() {
	...
	/**
	 * 说明： 在本测试中开了两个线程调用两个对象中的synchronized代码块，输出为交替打印输出。
	 * <p>
	...
	/**
	 * 说明： 在本测试中开了两个线程调用同一个对象中的synchronized修饰的方法， 输出为先打印第一个线程的输出，再打印第二个线程的输出。
	 */
	@Test
	...
	/**
	 * 说明： 在本测试中开了两个线程调用不同实列的synchronized修饰的方法，输出为第一个线程，第二个线程交替执行。
	 * 注意:父类中定义了synchronized修饰的方法，由子类继承时，需要在子类显示指定synchronized修饰。
	 * synchronized不属于方法声明的一部分，不能被继承。 结论：对比可以得出，synchronized修饰的方法作用域为在一个对象中。
	 */
	@Test
